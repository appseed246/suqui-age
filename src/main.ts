...
type OnTimerEvent = GoogleAppsScript.Events.TimeDriven;
...
function onEdit(e: OnEditEvent) {
  ...
  if (sheetName === e.source.getSheetName() && editedRange === monitoredRange) {
    if (sheet != null && targetSheet != null) {
...
function calendarApiExec(e: OnTimerEvent) {
  const sheet = SpreadsheetApp.getActiveSpreadsheet();
  sheet.getRange("A1").setValue("test");

  const sheet2 = sheet.getSheetByName("シート2");
  sheet2.getRange("A2").setValue("シート2のA2");
}